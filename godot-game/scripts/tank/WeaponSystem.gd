...
func _fire_barrel(i: int) -> void:
        ...
        # `offset` is already in the tank's local space; spawn_bullet() applies
        # to_global() itself, which already accounts for the tank's rotation, so
        # passing a pre-rotated offset here would double-apply rotation.
        tank.spawn_bullet(offset, dir, params)
