...
func _act_farm() -> void:
        # Uses the target cached by _think() (refreshed every THINK_INTERVAL)
        # instead of re-scanning every shape on every physics frame — with
        # thousands of shapes and dozens of bots that scan was the single
        # biggest source of frame lag.
        if farm_target == null or not is_instance_valid(farm_target):
                _act_wander(0.016)
                return
        var to_shape: Vector2 = farm_target.global_position - tank.global_position
